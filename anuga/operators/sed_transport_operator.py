...
#         Quantity(self.domain, name='energy', register=True)
        ...
        self.U = num.zeros((self.num_cells,))
...
#             self.domain.set_quantity('vel', self.U, location='centroids')
        ...
        if self.prev_t == 0:
            num.save('d_x_mid.npy', self.x)
            num.save('d_y_mid.npy', self.y)
            self.prev_t = 1
#         
#         
#         if round(t) == self.prev_t:
#             
#             self.prev_t = self.prev_t + 50
#             num.save('data/elev_' + str(num.round(t)) + '.npy', self.elev_c)
#             num.save('data/depth_' + str(num.round(t)) + '.npy', self.depth)
#             num.save('data/conc_' + str(num.round(t)) + '.npy', self.conc)
#             num.save('data/vel_' + str(num.round(t)) + '.npy', self.U)
# 
#             ss = self.domain.quantities['shear_stress'].centroid_values
#             num.save('data/ss_' + str(num.round(t)) + '.npy', ss)
    ...
    def update_bed(self, dzdt):
        ...
        if len(change[num.abs(change) > 0.001]) > 0:
        
#             print change[num.abs(change) > 0.001]
            change[num.abs(change) > 0.001] = num.sign(change[num.abs(change) > 0.001]) * 0.001
#             print change[num.abs(change) > 0.001]
#             print '---'
        

        self.elev_c[:] = self.elev_c + change
        self.depth[:] = self.depth - change
        
#         
#         #### patches ####
#         
#         if self.depth[self.depth - change <= 0].min() < 0:
#         
#             self.elev_c[self.depth - change <= 0] += self.depth[self.depth - change <= 0]
#             self.depth[self.depth - change <= 0] = 0
#             self.conc[self.depth - change <= 0] = 0
